




/* Courses*/
// export const COURSES =[
//   {
//     title:"UPSC Integrated Batch",
//     id:1,
//     links:[
//       "Targeted to crack the exam in the first attempt",
//       "Delhi Pattern ",
//       "-----------------------------------------------",
//       "---------------------------------------------",
//       "-----------------------------------------",
//       "--------------------Exam Oriented Systematic Approach",
//       "--------------------",
//       "-------------------------------",
//     ]
//   },
//   {
//   title:"UPSC Integrated Batch",
//   id:2,
//     links:[
//       "Targeted to crack the exam in the first attempt",
//       "Delhi Pattern ",
//       "-----------------------------------------------",
//       "---------------------------------------------",
//       "-----------------------------------------",
//       "--------------------Exam Oriented Systematic Approach",
//       "--------------------",
//       "-------------------------------",
//     ]
//   },
//   {
//   title:"UPSC Integrated Batch",
//   id:3,
//     links:[
//       "Targeted to crack the exam in the first attempt",
//       "Delhi Pattern ",
//       "-----------------------------------------------",
//       "---------------------------------------------",
//       "-----------------------------------------",
//       "--------------------Exam Oriented Systematic Approach",
//       "--------------------",
//       "-------------------------------",
//     ]
//   },
// ]






//for navbar
export const NAV_LINKS = [
    {href: '/', key: 'home', label: 'Home'},
    {href: '/' , key: 'Test_Series', label:'Test Series'},
    {href:'/', key:'class_program', label:'Class Program'},
    {href:'/',key:'about_us', label:'About Us'},
    {href:'/',key:'contact_us',label:'Contact Us'},

];





// for footer 
export const FOOTER_CONTACT_INFO = {
  
  title: "Contact Us",
  description:"Our centre provides absolute guidance in selecting the optional subjects and in every phase of the examination.",
  links: "contact@smcas.in",
  links2: "+91 8605954303",
    
  
  // links: [
    
  //   "contact@company.com",
  //   "(123) 456 -789",
    
  // ],
  label1:"Indira Institute of Business Management,",
  label2:" Plot No.2, Sector 9, ",
  label3:"Sanpada, Navi Mumbai, Maharashtra 400705",
};

export const FOOTER_LINKS = {
  title: "Quick Links",
  label: "Home",
  label2:"About Us",
  label3: "Contact Us",
  link1 :"Test Series", 
  link2:"Class Program", 
};

export const SOCIAL = {
  title: "Social",
  description:
    "To be the Centre of Excellence to ensure our students are successful and that they achieve their aspirations within a timeframe",
  // links: ["/facebook.svg", "/instagram.svg"],
  link1:"/facebook.svg", 
  link2:"/instagram.svg",
};
